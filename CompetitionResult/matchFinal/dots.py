...
promoteCodeList = input('promoteCodeList: ')
for plrname in PLRS:
    if plrname[6] in promoteCodeList:
        plt.plot(PLRS[plrname][1], PLRS[plrname][2], 'bo', markersize=1)
        avgX, avgY = np.mean(PLRS[plrname][1]), np.mean(PLRS[plrname][2])
        plt.plot(avgX, avgY, 'r^', markersize=11)
        plt.annotate(str.upper(plrname[6]), xy=(avgX, avgY), xytext=(avgX, avgY),
                     fontsize='xx-large')
    else:
        plt.plot(PLRS[plrname][1], PLRS[plrname][2], 'bo', markersize=1)
